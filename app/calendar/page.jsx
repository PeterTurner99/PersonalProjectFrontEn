...
async function update_recipe_search(url, { arg }) {
    ...
    let error = undefined
    console.log(res_json, "test test")
    if (!res.ok) {
...
function PopoverContents({ shown }) {
    ...
    if (!recipe_data && !recipe_error && showForm) {
        recipe_trigger('')
    }
    return {
        popover_body: (<div>
            ...
            <div className={showForm ? 'block mt-5' : 'hidden'}>
                <FormAndMethod setOpen={setShowForm} trigger={trigger} keyValue={date} method={'POST'} url={`${CALENDAR_URL}?date=${date}`} className={'flex flex-col gap-4'}>
                    <div className={'flex flex-col gap-2'}>
                        <label htmlFor="recipe">Recipe</label>
                        <Input type={'text'} id={'recipe'} value={recipe} required name={'recipe'} onChange={value => {
                            setRecipeSearch(value.target.value);
